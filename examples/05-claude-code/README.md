...
- You've read Tasks 02, 03, and 04, you've already picked Claude Code as your tool, and your account can log in and send messages normally.
...
This is a perfect illustration of the framework from Task 03: the same brain, wrapped in two different shells. The Home layer is a ChatBot — you ask, it answers. The Code layer is an Agent — it can read your files, change your files, and work through a task step by step on its own. Switching over is literally you swapping shells.
...
**Before you touch anything, do one thing first: clone this teaching repo locally.** Claude Code needs a folder to work in — without one, it has nothing to act on. This is exactly what Task 02 meant by "one job equals one folder, and that folder is a repo."
...
Remember what Task 03 said: what you're paying for is compute, so "good enough" is a real cost-saving strategy here, not a compromise.
...
Now it's time to cash in the setup from Task 02: actually use an **Agent Skill** with your own hands.
...
**Look at its path and it all clicks: an Agent Skill is fundamentally just a file that defines this capability.** Task 02 mentioned that a Skill can also bundle reference material, scripts, tools, and so on — that's all true, but this example is kept as minimal as possible: just this one file.
...
This is exactly what Task 02 meant by "building an expert." You think through, once, how a particular kind of task should be done, package it as a Skill, and from then on every summary automatically hits that same bar — you never have to spell it out again.
...
This is exactly why Task 02 lists GitHub as a hard prerequisite: you need somewhere to keep this stuff, somewhere with enough room, that you can find again and edit.
